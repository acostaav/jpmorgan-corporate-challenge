...
//            node.registerInitiatedFlow(JPMorganFlow.Acceptor.class);
//        }
//        network.runNetwork();
//    }
//
...
//        // The JPMorganContract specifies that IOUs cannot have negative values.
//        JPMorganFlow.Initiator flow = new JPMorganFlow.Initiator(-1, b.getInfo().getLegalIdentities().get(0));
...
//        Integer iouValue = 1;
//        JPMorganFlow.Initiator flow = new JPMorganFlow.Initiator(iouValue, b.getInfo().getLegalIdentities().get(0));
//        CordaFuture<SignedTransaction> future = a.startFlow(flow);
//        network.runNetwork();
//        SignedTransaction signedTx = future.get();
//
...
//
//            JPMorganState recordedState = (JPMorganState) txOutputs.get(0).getData();
...
//        for (StartedMockNode node : ImmutableList.of(a, b)) {
//            node.transaction(() -> {
//                List<StateAndRef<JPMorganState>> ious = node.getServices().getVaultService().queryBy(JPMorganState.class).getStates();
//                assertEquals(1, ious.size());
//                JPMorganState recordedState = ious.get(0).getState().getData();
//                assertEquals(recordedState.getValue(), iouValue);
//                assertEquals(recordedState.getLender(), a.getInfo().getLegalIdentities().get(0));
//                assertEquals(recordedState.getBorrower(), b.getInfo().getLegalIdentities().get(0));
//                return null;
